...
const checkWeather = (lat, lon, hoursfromnow, CHANNELID) => {
	const weatherData =
		'https://opendata.fmi.fi/wfs?service=WFS&version=2.0.0&request=getFeature&storedquery_id=fmi::forecast::harmonie::surface::point::simple&latlon=' +
		lat +
		',' +
		lon +
		...
		fetch(weatherData)
			...
			.then(async (xml) => {
				...
				for (let i = 0; i < elements.length; i++) {
					const paramName = elements[i].getElementsByTagName(
						'BsWfs:ParameterName'
					)[0].textContent;
					if (paramName === 'WeatherSymbol3' && i === hoursfromnow * 3 - 1) {
						let paramValue = elements[i].getElementsByTagName(
					...
					if (paramName === 'temperature' && i === hoursfromnow * 3 - 3) {
						const paramValue = elements[i].getElementsByTagName(
							'BsWfs:ParameterValue'
						)[0].textContent;
